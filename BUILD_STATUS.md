...
| `client/shaders/{vs_chunk,fs_chunk}.sc` + `engine/rendering::load_chunk_program` | **TESTED** (headless draw call, real shaderc compile) / **NOT VERIFIED — ENVIRONMENT LIMITATION** (visual) | Real per-block/per-face color (Phase 26, `BlockDefinition::color`/`side_color`/`bottom_color`) plus a directional light (with a small top-face lift) and a subtle deterministic-per-voxel hash-noise pattern - still no texture atlas (Phase 12). Compiled via bgfx's `shaderc` when `LCU_BUILD_SHADER_TOOLS=ON` (opt-in, see BUILDING.md/DECISIONS.md - pulls in glslang/SPIRV-Tools/SPIRV-Cross/Dawn-Tint). Loaded into a real `bgfx::ProgramHandle` and submitted via `Renderer::submit_chunk_mesh()` -> `bgfx::submit()` every frame. Verified via a real `LCU_BUILD_SHADER_TOOLS=ON` build and run: `"Chunk shader program valid=true"` (the new `Color0` attribute/varying/fragment logic all link through the real bgfx pipeline), plus a real headless `LCU_VERIFY_BREAK_PLACE` run under that build showing zero regressions - under the `Noop` backend (no GPU/display in this sandbox). **Not verified, no way around it here**: what it actually looks like on a real GPU/display. Without `LCU_BUILD_SHADER_TOOLS=ON`, `VoxelClient` still runs fine and logs `valid=false` for the program, skipping the draw (no crash). |
