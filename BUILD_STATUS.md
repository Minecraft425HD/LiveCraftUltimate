...
| `engine/rendering::upload_chunk_mesh_layer`/`GpuChunkMesh` | **TESTED** (buffer creation) / **NOT IMPLEMENTED** (draw call) | Creates real bgfx `VertexBufferHandle`/`IndexBufferHandle` from a `ChunkMeshLayer`. 3 unit tests, plus a real `VoxelClient` run confirming `valid=true index_count=36` end-to-end (chunk -> job-dispatched mesh -> GPU buffers). **No shader program or `bgfx::submit()` draw call exists yet** — bgfx requires a compiled shader to draw anything, and this repo has no shader compiler (shaderc) built. Buffers are created and destroyed correctly; nothing is drawn to the screen yet. This is the next task, not a gap papered over. |
